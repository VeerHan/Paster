...
- **🔍 搜索**：快速搜索历史记录
- **📌 置顶管理**：常用条目可置顶，新增内容会自动排在置顶组之后
- **🧭 最近使用优先**：点击某条历史后会移动到最前面，方便连续使用
- **🗑️ 清空历史**：清空时删除全部历史并自动清理图片缓存
...
5. **置顶/删除**：鼠标悬停条目后，点击右上角系统菜单可置顶、取消置顶或删除
6. **置顶排序规则**：取消置顶后，条目会按创建时间回到普通条目区域，而不是停留在原地
...
会在 `dist/` 下生成：
- **Paster-1.0.3.dmg**：标准 macOS 安装镜像，双击后把 Paster 拖到 Applications 即可
